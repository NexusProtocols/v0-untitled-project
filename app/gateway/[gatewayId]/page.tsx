...
export default function GatewayPage() {
  ...
  const [normalizedSteps, setNormalizedSteps] = useState<GatewayStep[]>([])
  ...
  // Default task types to use for filling in missing tasks
  const defaultTaskTypes: StepType[] = ["redirect", "article", "youtube", "operagx", "direct"]
  ...
  useEffect(() => {
    const fetchGateway = async () => {
      try {
        ...
        // Normalize steps to always have exactly 5
        normalizeSteps(currentGateway.steps)
      } catch (error) {
  ...
  // Normalize steps to always have exactly 5
  const normalizeSteps = (steps: GatewayStep[]) => {
    let normalized = [...steps]

    // If we have fewer than 5 steps, add placeholder steps
    if (normalized.length < 5) {
      for (let i = normalized.length; i < 5; i++) {
        normalized.push({
          id: `placeholder-${i}`,
          type: defaultTaskTypes[i % defaultTaskTypes.length],
          title: `Task ${i + 1}`,
          description: `Complete this task to continue`,
          content: {
            url: "https://example.com",
            videoId: "dQw4w9WgXcQ", // Rick roll as placeholder
          },
        })
      }
    }

    // If we have more than 5 steps, truncate to 5
    if (normalized.length > 5) {
  ...
  useEffect(() => {
    if (normalizedSteps.length > 0 && completedTasks.length === normalizedSteps.length) {
  ...
  }, [completedTasks, normalizedSteps, currentStage])
  ...
  const handleClaimReward = async () => {
    ...
    try {
      ...
      if (gateway?.reward?.type === "url" && gateway?.reward?.url) {
        ...
        const redirectUrl = `${gateway.reward.url}${separator}token=${data.token || "completed"}`
              ...
              <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="w-full md:w-2/3">
                  ...
                  <div className="flex flex-wrap items-center gap-3 text-sm">
                    ...
                    <span className="rounded bg-[#050505] px-2 py-1 text-gray-300">
                      <i className="fas fa-tasks mr-1"></i> 5 Tasks
                    </span>
                  </div>
                </div>
              </div>
            ...
            {!showFinalReward ? (
              <>
                ...
                <div className="mb-6">
                  <div className="flex items-center justify-between mb-2">
                    <div className="text-sm text-white font-medium">
                      {completedTasks.length} of {normalizedSteps.length} Tasks Completed
                    </div>
                    <div className="text-sm text-gray-400">
                      {Math.round((completedTasks.length / normalizedSteps.length) * 100)}% Complete
                    </div>
                  </div>
                  <div className="h-2 w-full bg-[#111] rounded-full overflow-hidden">
                    <div
                      className="h-full bg-gradient-to-r from-[#ff3e3e] to-[#ff0000]"
                      style={{ width: `${(completedTasks.length / normalizedSteps.length) * 100}%` }}
                    ></div>
                  </div>
                </div>
                ...
                <div className="mb-8 space-y-4">
                  {normalizedSteps.map((step: GatewayStep, index: number) => (
                    <GatewayTaskButton
                      key={step.id}
                      taskType={step.type}
                      taskNumber={index + 1}
